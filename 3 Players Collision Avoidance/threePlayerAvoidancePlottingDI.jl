...
function plotter(str::AbstractString)
    ...
    players = 3
    iter = 2*players
    initial_pose1 = [-1,0.5,0,0]
    final_pose1 = [1,0.5,0,0]
    initial_pose2 = [0,-1,0,0]
    final_pose2 = [0,1,0,0]
    initial_pose3 = [-1,-0.5,0,0]
    final_pose3 = [1,-0.5,0,0]
    ...
    max = maximum([maximum([maximum([x1_domain[2], y1_domain[2]]), maximum([x2_domain[2], y2_domain[2]])]),maximum([x3_domain[1], y3_domain[1]])])
    domain = [min,max];
...
plotter("3playertrajectory_2.txt")
