...
obstacle = [0.1,0.2,0.3,0.1,0.2,0.3] # (1,2), (1,3), (1,4),(2,3),(2,4),(3,4)
z_guess = zeros(40 * T + 16)
total = 4
...
function dynamic_feasibility(z, model, T)
    ...
    for a in 1:total
        ...
        elseif a == 3
            ...
            h_z[1+(4*T+4)*(a-1)] = (x[1] - (initial_pose4[1] + dt * initial_pose4[3]))
            h_z[2+(4*T+4)*(a-1)] = (x[2] - (initial_pose4[2] + dt * initial_pose4[4]))
            h_z[3+(4*T+4)*(a-1)] = (x[3] - (initial_pose4[3] + dt * u[1]))
            h_z[4+(4*T+4)*(a-1)] = (x[4] - (initial_pose4[4] + dt * u[2]))
        end
        ...
        elseif a == 3
            ...
            h_z[4*T+1+(4*T+4)*(a-1)] = x[4*T-3] - final_pose4[1]
            h_z[4*T+2+(4*T+4)*(a-1)] = x[4*T-2] - final_pose4[2]
            h_z[4*T+3+(4*T+4)*(a-1)] = x[4*T-1] - final_pose4[3]
            h_z[4*T+4+(4*T+4)*(a-1)] = x[4*T] - final_pose4[4]
        end
    end
...
function optimizer(file, α)
    ...
    for j in 2:T+1
        ...
        # Player 4
        states[4, 1, 1] = initial_pose4[1]
        states[4, 1, 2] = initial_pose4[2]
        states[4, 1, 3] = initial_pose4[3]
        states[4, 1, 4] = initial_pose4[4]
        for j in 2:T+1
            states[4, j, 1] = value(z[4*(j-1)-3+3*6*T])
            states[4, j, 2] = value(z[4*(j-1)-2+3*6*T])
            states[4, j, 3] = value(z[4*(j-1)-1+3*6*T])
            states[4, j, 4] = value(z[4*(j-1)+3*6*T])
        end
...
α = [0.01]
for i in α
    optimizer("4playertrajectory.txt",i)
end